...
| GET | `/api/quotes?symbols=&date=` | 최근 영업일 종가·등락률(포트폴리오 평가용). `date` 미지정 + 평일 09:00~18:00 KST 면 네이버 금융 실시간(비공식) 값으로 덮어쓰고(`live: true`), DB 시세가 없는 국내 종목도 실시간 값만으로 채운다 |
...
- **워치리스트 밖 보유 종목도 장중엔 시세가 붙습니다**: DB 시세 행이 없는 국내 종목은 네이버
  ...
  해외 티커(`AAPL`)는 이 엔드포인트가 모르므로 호출 전에 걸러냅니다.
...
- 해외 종목(그리고 워치리스트 밖 국내 종목)은 토스가 함께 내려주는 현재가·등락률을
  `importedClose`/`importedChangePct` 로 같이 저장합니다. 대시보드는 워치리스트 실시간 시세가
  있으면 그걸 우선하고, 없을 때만 이 값을 "가져온 시세"로 표시합니다 — **가져오기 시점 스냅샷**이라
  다시 동기화하기 전까지는 갱신되지 않습니다.
