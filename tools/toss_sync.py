...
def to_holdings(overview: dict, include_us: bool = False) -> tuple:
    """HoldingsOverview → (holdings, skipped) 로 변환.

    기본값은 **국내(KRW) 종목만**이다. 대시보드의 평가·합계는 원화 기준이고
    시세도 KRX 만 들어오므로, 달러 종목을 섞으면 매입금액 합계가 통화가 뒤섞인
    엉터리 숫자가 된다. --include-us 로 켜면 포함하되 호출부가 경고를 띄운다.
    ...
    for item in (overview or {}).get("items") or []:
        ...
        if not is_krw and not include_us:
            skipped.append(item.get("name") or item.get("symbol") or "?")
            continue
        holding = to_holding(item)
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        help="해외(USD) 종목도 포함. 대시보드 합계는 원화 기준이라 통화가 섞입니다",
    )
    ...
    holdings, skipped = to_holdings(overview, include_us=args.include_us)
